...
while read -r in; do echo "==> Installing Ruby $in";rvm install ruby-"$in"; done < "$HOME"/.templates/dependencies/rubies
source ~/.bashrc
rvm use ruby-"$(cat "$HOME"/.templates/dependencies/ruby-default)" --default
rvm list
while read -r in; do gem install "$in"; done < "$HOME"/.templates/dependencies/gem
